	...
	public List<Lander> generateLanders(List<Planet> planets, int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			do {
				...
				float x = (float) Math.cos(angleOnPlanet * Math.PI / 180) * planets.get(planetNumber).getRadius() + planets.get(planetNumber).getPosition().x;
				float y = (float) Math.sin(angleOnPlanet * Math.PI / 180) * planets.get(planetNumber).getRadius() + planets.get(planetNumber).getPosition().y;
				Lander lander = new Lander(planets.get(planetNumber), new Vector(x, y));
				landers.add(lander);
			} while (!isNewPlanet);
		}
